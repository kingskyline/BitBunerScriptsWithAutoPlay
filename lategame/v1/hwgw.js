...
export async function main(ns) {
  ...
  const RAM_PER_THREAD = ns.getScriptRam("/lategame/shared/weaken.js"); // RAM requirement per thread.
  const RESERVED_HOME_RAM = 200; // Amount of RAM reserved on the 'home' server.
  ...
  const LOOP_DELAY = 1000 * 1; // Delay after completing an iteration over all targets and servers.
    ...
    for (let target of TARGETS) {
      ...
      for (let server of SERVERS) {
        ...
        if (security_level > min_security_level) {
          ...
          ns.exec("/lategame/shared/weaken.js", server, available_threads, target, 0, batch_number++);
          ...
          ns.exec("/lategame/shared/grow.js", server, available_threads, target, 0, batch_number++);
          ...
          ns.exec("/lategame/shared/weaken.js", server, weaken_threads, target, batch_delay, batch_number);
          ns.exec("/lategame/shared/weaken.js", server, weaken_threads_2, target, batch_delay + BATCH_THREAD_DELAY * 2, batch_number);
          ns.exec("/lategame/shared/grow.js", server, grow_threads, target, batch_delay + BATCH_THREAD_DELAY + ns.getWeakenTime(target) - ns.getGrowTime(target), batch_number);
          ns.exec("/lategame/shared/hack.js", server, hack_threads, target, batch_delay - BATCH_THREAD_DELAY + ns.getWeakenTime(target) - ns.getHackTime(target), batch_number++);
