...
export async function main(ns) {
  let servers = [];
  let ramPerThread = ns.getScriptRam("/Midgame/BoughtServerHacker.js");
  ...
  while (serversToScan.length > 0) {
    ...
    if (!servers.includes(server) && server !== "home" && server.match(/^server-(?:[0-9]|1\d|2[0-4])$/)) {
      ...
      if (ns.hasRootAccess(server)) {
        await ns.scp("/Midgame/BoughtServerHacker.js", server);
        ...
        if (threads > 0) {
          ...
          ns.exec("/Midgame/BoughtServerHacker.js", server, threads, target);
