...
export async function main(ns) {
  let servers = [];
  let ramPerThread = ns.getScriptRam("early/BoughtServerHackerXP.js");
  ...
  while (serversToScan.length > 0) {
    ...
    if (!servers.includes(server) && server !== "home" && server.match(/^server-(?:[0-9]|1\d|2[0-4])$/)) {
      ...
      if (ns.hasRootAccess(server)) {
        await ns.scp("early/BoughtServerHackerXP.js", server);
        let ramAvailable = ns.getServerMaxRam(server) - ns.getServerUsedRam(server);
        let threads = Math.floor(ramAvailable / ramPerThread);
        textblock += `server ${server} has root,`;

        if (threads > 0) {
          ns.exec("early/BoughtServerHackerXP.js", server, threads, target, "grow");
